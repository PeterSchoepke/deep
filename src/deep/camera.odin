...
camera_init :: proc(position: Vec3) {
...
set_camera_position :: proc(position: Vec3) {
    camera.position = position;
}

camera_get_view_matrix :: proc() -> Mat4 {
    return linalg.matrix4_look_at_f32(camera.position, camera.position + camera.front, camera.up)
}

camera_process_keyboard :: proc(forward, back, left, right, up, down: bool, delta_time: f32) {
...
camera_process_mouse_movement :: proc(xoffset, yoffset: f32, constrain_pitch: bool) {
