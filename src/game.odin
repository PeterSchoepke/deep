...
load_meshes :: proc(render_context: ^Render_Context, meshes: ^Meshes) {
...
destroy_meshes :: proc(render_context: ^Render_Context, meshes: ^Meshes) {
    for i in 0..<meshes.count {
        destroy_render_data(render_context, &meshes.data[i])
    }
}

load_lights :: proc(lights: ^Lights) {
